...
// === ПОЛУЧЕНИЕ ДАННЫХ ПОЛЬЗОВАТЕЛЯ ===
  ...
  router.get('/websites', (Request request) async {
    final userIdStr = request.url.queryParameters['user_id'];
    final userId = int.tryParse(userIdStr ?? '');
  ...
  router.get('/network-connections', (Request request) async {
    final userIdStr = request.url.queryParameters['user_id'];
    final userId = int.tryParse(userIdStr ?? '');
    ...
    try {
      ...
      final connections = result.map((row) {
        ...
        return {
          ...
          'network_connection_description': raw['network_connection_description'],
          'ipv4': raw['ipv4'],
          'ipv6': raw['ipv6'],
          'password_hash': raw['password_hash'],
          'salt': raw['salt'],
      ...
      print(stack);
      return Response.internalServerError(
        body: jsonEncode({'error': 'Ошибка сервера при получении сетевых подключений'}),
        headers: {'Content-Type': 'application/json'},
      );
    }
  });
